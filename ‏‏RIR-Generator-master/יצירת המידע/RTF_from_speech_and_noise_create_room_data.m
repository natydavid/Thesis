...
if strcmp(exp_name,'vol_basic_cubic')
    ...
    save_name = 'vol_basic_cubic_data_speech_RTF.mat';
else
    load_noise_exp = [exp_name '_noise_RIR.mat'];
    load_exp = [exp_name '.mat'];
    save_name = [exp_name '_speech_and_noise_RTF.mat'];
end

%% load room expriment
load(load_noise_exp,'room_rir_list');
...
[room_noise_rir_list,room_list] = pickRooms( room_list,room_noise_rir_list,vol,shp,beta );

if par_calc
    room_speech_RTF_list = rir2speechAndNoise2RTF( room_rir_list,room_noise_rir_list,room_list,speech_file,save_name,nworkers );
else
    room_speech_RTF_list = rir2speechAndNoise2RTF( room_rir_list,room_noise_rir_list,room_list,speech_file,save_name );
end
