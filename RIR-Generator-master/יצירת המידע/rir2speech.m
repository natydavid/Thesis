function [ room_speech_list ] = rir2speech( room_rir_list,room_list,speech_file,save_name,nworkers )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
rir_sz = size(room_rir_list);
room_speech_list = cell(rir_sz);
...
for vlvl = 1:vol_cls
    for spsp = 1:shp_cls
        for bb = 1:beta_num
            room_speech_list{vlvl,spsp,bb} = cell(size(room_rir_list{vlvl,spsp,bb}));
        end
    end
end
...
for vlvl = 1:vol_cls
    for spsp = 1:shp_cls
        for bb = 1:beta_num
            ...
            [num_spkrPos,num_of_dis] = size( room_speech_list{vlvl,spsp,bb});
                ...
                if exist('nworkers','var')
                    ...
                    parfor (dd = 1:num_of_dis,nworkers)
                        if ~isempty(thisRirs{dd})
                            temp{dd} = conv2(1,speech_anechoic,thisRirs{dd});
                        end
                    end
                    room_speech_list{vlvl,spsp,bb}(ss,:) = temp;
                else
                    for dd = 1:num_of_dis
                        if ~isempty(room_rir_list{vlvl,spsp,bb}{ss,dd})
%                             room_rir_list{vlvl,spsp,bb}{ss,dd} = [zeros(5,2000) ones(5,1)];
                            room_speech_list{vlvl,spsp,bb}{ss,dd} = conv2(1,speech_anechoic,room_rir_list{vlvl,spsp,bb}{ss,dd});
                        end
                    end
                end             
...
if exist('save_name','var')
    save(save_name,'room_speech_list');
