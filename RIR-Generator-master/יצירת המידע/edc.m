function [rt60, curve] = edc(h, fs, start_time, finish_time)
% function Y= edc(h,fs, start_time, finish_time)
% estimates the reveberation time (RT_60) by performing
% a linear fit to a segement of the enrgy decay curve (edc).
%
% Input parameters:
...
plot([tt(start), tt(finish)],...
     coef(2) + coef(1) * [tt(start), tt(finish)],'c','linewidth',3); % plosts fitted line.
hold on;
plot (tt, cs,'b'); % plot energy decay curve.
hold on;

ih = cs(1);      %interval height on plot
plot ([tt(start), tt(finish)],[ih, ih],'r');
title('normalized EDC - verify that red line is above linear section');
axis([0, tt(end), cs(end), ih+5]);
xlabel('time [sec]');
ylabel('normalized EDC [dB]');
grid on
hold off;
